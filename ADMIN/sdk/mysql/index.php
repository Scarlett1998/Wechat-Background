...
class php_mysql 
{
    ...
    */
    ...
    function getsip()
	{
	 	  $str = file_get_contents('http://pv.sohu.com/cityjson');
    ...
    function inject_check($sql_str) { //防止注入
        ...
        if ($check) {
            echo "警告：安全检查不通过，检测到SQL注入迹象！";
            exit ();
        } else {
            return $sql_str;
        }
    }
    function checkurl($URL) { //检查来路
	...
	function try_sip()
	{
	    $url="../../IP/GetIP.php";
		$public_key=Get256();
		$private_key=md5("IPMD5".$public_key);
		$header= $header= array(
              'Content-Type: multipart/form-data; ',
              
             );
		$content['public_key']=$public_key;$content['private_key']=$private_key;
		$data=send_post($url, $header, $content);
		if($data['state']!='ok') return "error";
		return $data['IP'];
	}
